...
public abstract class PrestitoAbstractDAO extends AbstractJDBCDAO {
    ...
    public void create(Prestito prestito) throws DAOCreateException {
        ...
        try {
            ...
            query.append("   (BENEFICIARIO, OGGETTO_PRESTATO, DATA_SCADENZA, DATA_PRESTITO) ").append(EOL);
    ...
    public void update(Prestito prestito) throws DAOStoreException {
        ...
        try {
            ...
            query.append(" data_scadenza = ? , data_prestito = ?  ").append(EOL);
    ...
    protected boolean isDirty(Prestito oldPrestito, Prestito newPrestito) {
        ...
        if (oldPrestito.getDataPrestito() == null && newPrestito.getDataPrestito() != null)
            return true;
        if (oldPrestito.getBeneficiario() != null && !oldPrestito.getBeneficiario().equals(newPrestito.getBeneficiario()))
            return true;
        if (oldPrestito.getBeneficiario() == null && newPrestito.getBeneficiario() != null)
            return true;
    ...
    public static Prestito cloneWithoutChildren(Prestito prestito) {
        ...
        newPrestito.setBeneficiario(prestito.getBeneficiario());
     ...
     */
    public Prestito get(String beneficiario_username, Integer oggettoPrestato_id) throws DAOFinderException {
        Connection connection = null;
        PreparedStatement statement = null;
        ResultSet rs = null;
        try {
            ...
            throw new DAOFinderException(FrameworkStringUtils.concat("Cannot find Prestito on database with [beneficiario_username = ", beneficiario_username,
                " oggettoPrestato_id = ", oggettoPrestato_id, "]"));

        } catch (SQLException ex) {
            String msg =
                FrameworkStringUtils.concat(
                    "Error on method get(String beneficiario_username, Integer oggettoPrestato_id) for Prestito on database with [beneficiario_username = ",
    ...
    public Prestito buildModelFromResultSet(ResultSet rs, QborrowManager qborrowManager) throws SQLException {
        ...
        prestito.setBeneficiario_username(getParameterString(rs, "beneficiario"));
    ...
    public void delete(String beneficiario_username, Integer oggettoPrestato_id) throws DAODeleteException {
        ...
        try {
            ...
            if (numberOfDeletedRecord < 1) {
                String msg =
                    FrameworkStringUtils.concat("No record of Prestito deleted with params [beneficiario_username = ", beneficiario_username,
                        " oggettoPrestato_id = ", oggettoPrestato_id, "]");
                if (log.isWarnEnabled()) {
                    log.warn(msg);
                }
            ...
            String msg =
                FrameworkStringUtils.concat("Error during delete records on Prestito with params [beneficiario_username = ", beneficiario_username,
                    " oggettoPrestato_id = ", oggettoPrestato_id, "]");
            if (log.isErrorEnabled()) {
                log.error(msg);
            }
    ...
    /**
     * Get list of Prestito by Beneficiario
     * 
     * @param Beneficiario
     * @return a Prestito list
     */
    public List<Prestito> getPrestitoListByBeneficiario(String beneficiario_username) {
        ...
        try {
            ...
            String msg = FrameworkStringUtils.concat("Unexpeted error on find Prestito with beneficiario_username = ", beneficiario_username, "on database.");
            if (log.isErrorEnabled()) {
                log.error(msg, ex);
            }
            throw new SystemException(msg, ex);
    ...
    /**
     * Delete Prestito list by Beneficiario
     * 
     * @param beneficiario_username
     * @throws DAODeleteException if no record deleted
     */
    public void deletePrestitoListByBeneficiario(String beneficiario_username) throws DAODeleteException {
        Connection connection = null;
        PreparedStatement statement = null;
        try {
            // Compose the select query
            ...
            if (numberOfDeletedRecord < 1) {
                String msg =
                    FrameworkStringUtils.concat("An error occurred delete Prestito with beneficiario_username = ", beneficiario_username,
            ...
            String msg =
                FrameworkStringUtils.concat("Unexpeted error on delete Prestito with beneficiario_username = ", beneficiario_username, " on database.");
            if (log.isErrorEnabled()) {
                log.error(msg, ex);
            }
            throw new SystemException(msg, ex);
    ...
    protected void addOrderClause(PrestitoSearch search, StringBuilder query) {
        switch (search.getOrder()) {
            ...
            default:
                query.append(" ORDER BY data_scadenza DESC, data_prestito ASC");
                break;
        }
    }
